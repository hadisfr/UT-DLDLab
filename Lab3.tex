...
and the \textit{vertical resolution}, the smallest voltage increment that can be programmed in a signal source.\todo{Is this true? is this resolution?}
...
Accordingly, Below is the topics that are explained in the following of this experiment in details:\todo{Should be removed? There is a complete Table of Contents in the previous page. Or should be linked to each section?}
\begin{itemize}
    \item DAC
...
\Fref{fig:PMW}\todo{?} shows sample PWM waves. In each cycle, first \textit{PW} clock output value is 1 and it is 0 for rest of cycle.
...
\begin{enumerate}
    ...
    \item Generation of sine function can be somewhat different. The following second order differential equation can be used to generate the sine function:\todo{Check these, please.}
    \begin{gather*}
    ...
    Refer to the Appendix~A\todo{Which Appendix?} to learn about ROM memories and the way they are generated and initialized through MegaWizard in Quartus~II. \Fref{fig:WGblockdia} shows the block diagram of the waveform generator module.
\end{enumerate}
...
Type of amplitude is selected by a 2-bit input according to \fref{tab:ampsel}.\todo{\texttt{func[1:0]}? or amp?}
...
Expand the hierarchy in the Libraries box as shown in the \fref{fig:qblockdia}\todo{this one?}. First expand libraries, then expand the library primitives, followed by expanding the library logic which comprises the logic gates. Use the gates if any is needed.
