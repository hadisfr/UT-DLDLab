...
\begin{enumerate}
    ...
    \item Use the \ul{Persian alphabet ASCII codes}\todo{What does \textit{Persian ASCII codes} mean? Does it mean \href{https://en.wikipedia.org/wiki/Iran_System_encoding}{Iran System encoding}? Should more description be provided?} instead of the English ones and write them in \path{mif} file. Then do the same as \nameref{sec:imp} part and show the characters on the monitor.
\end{enumerate}
...
\begin{enumerate}
    \item Read about the \path{*.mif} files structure
